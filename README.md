...
```bash
npm install
npm run dev       # http://localhost:4321/DeFiYieldReview/
...
This repo is prepared for SEO coverage around DeFi yield platforms. Starter articles live under `src/content/reviews/` and use review/comparison schema, RSS, JSON feed, sitemap, and base-path-safe internal links.
...
Push this project to a GitHub repository named `DeFiYieldReview` and enable GitHub Pages from Actions. The workflow at `.github/workflows/deploy.yml` builds and publishes `dist/` automatically.
